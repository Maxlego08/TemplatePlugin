...
public class CommandManager implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command command, String s, String[] strings){
        ...
        sender.sendMessage(TextUtil.color(main.getPrefix() + " &cCette commande n'existe pas ou votre syntaxe contient une erreur"));	
		return true;
	}
	
	 private void processRequirements(VCommand command, CommandSender sender, String[] strings) {
        if (!(sender instanceof Player) && command.isNoConsole()) {
            sender.sendMessage(TextUtil.color(main.getPrefix() + " &eVous devez être un joueur pour faire cette commande"));
            return;
        }
        if (command.getPermission() == null || sender.hasPermission(command.getPermission())) {
        	...
        	CommandType returnType = command.perform(main, sender, strings);
             if (returnType == CommandType.SYNTAX_ERROR) {
                 sender.sendMessage(TextUtil.color(main.getPrefix() + " &eVous devez faire la commande comme ceci &6%command%".replace("%command%", command.getSyntax())));
             }
            return;
        }
        sender.sendMessage(TextUtil.color(main.getPrefix() + " &cVous n'avez pas la permission de faire cette commande ! Vous pouvez en obtenir avec le /boutique, sur le site ou avec l'Hôtel des ventes !"));	
